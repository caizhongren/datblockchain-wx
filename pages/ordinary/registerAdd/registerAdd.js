...
Page({
        ...
        if(res.data){
          console.log(res.data);
            ...
            for (let i = 0; i < res.data.aliveNumber; i++){
              piglets[i] = { earNotch: undefined, earTag: undefined, pigGender:0};
            }
            that.setData({
              register: res.data,
              piglets: piglets
  ...
  saveAddress(){
    ...
    console.log(piglets);
    console.log(register.id);//窝号
    console.log(data.columnCode)//蓝号
    console.log(register.sowId);//母id
    console.log(register.boarId);//父id
    console.log(register.childbirthTime);//出生日期
    console.log(data.varieties);//品种
